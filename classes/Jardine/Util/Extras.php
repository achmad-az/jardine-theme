...
class Extras
{
    ...
    {
        add_filter('body_class', array($this, 'setThemeBodyClasses'));

        add_action('wp_head', array($this, 'setPingbackHeader'));

        add_filter('manage_ize_gallery_posts_columns', array($this, 'eventTableHead'));

        add_action('manage_ize_gallery_posts_custom_column', array($this, 'eventTableContent'), 10, 2);

        add_filter('manage_ize_gallery_sortable_columns', array($this, 'eventTableSorting'));

        add_filter('siteorigin_panels_widget_dialog_tabs', array($this, 'customGroup'), 20);

        add_action('wp_dashboard_setup', array($this, 'dashboardWidget'));
    }

    public function setThemeBodyClasses($classes)
    {
        // $classes = array();
    ...
    public function setPingbackHeader()
    {
    ...
    public function eventTableHead()
    {
        $new = array();
    ...
    public function eventTableContent($column_name, $post_id)
    {
    ...
    public function eventTableSorting($columns)
    {
        $columns['category'] = 'Category';
        return $columns;
    }

    public function customGroup($tabs)
    {
        $tabs[] = array(
            'title' => __('jardine Widget', 'jardine'),
            'filter' => array(
                'groups' => array('jardine')
            )
        );
        return $tabs;
    }
    ...
    {
        wp_add_dashboard_widget('dashboard_widget', 'SETUP THEME', function() {
            ob_start();
            ?>
                <p>
